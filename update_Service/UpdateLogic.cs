...
namespace update_Service
{
    public class UpdateLogic
    {
        ...
        private async Task GetFile(Registry.Registry.RegistryClient client, Registry.Definition def, CancellationToken token)
        {
            var first = def;
            ...
            try
            {
                await foreach (Registry.Fragment chunkMsg in call.ResponseStream.ReadAllAsync(token))
                    await fs.WriteAsync(chunkMsg.Chunk.ToByteArray(), token);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex);
        ...
        public async Task UpdateCycle(Registry.Registry.RegistryClient client, CancellationToken token)
        {
            ...
            if (changedNeeded.Any())
            {
                foreach (var chg in changedNeeded)
                {
                    ...
                    await GetFile(client, chg, token);
